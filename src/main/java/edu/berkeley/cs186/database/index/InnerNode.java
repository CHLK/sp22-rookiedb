...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        while (node instanceof InnerNode) {
            node = getLowerNode(key, (InnerNode) node);
        }
        return (LeafNode) node;
    }

    private BPlusNode getLowerNode(DataBox key, InnerNode node) {
        int keySize = node.getKeys().size();
        if (keySize == 0) {
            return null;
        }
        Long pageNum;
        ...
        while (true) {
            if (right - left <= 1) {
                DataBox leftDataBox = node.getKeys().get(left);
                if (key.compareTo(leftDataBox) < 0) {
                    pageNum = node.getChildren().get(left);
                    break;
                }
                DataBox rightDataBox = node.getKeys().get(right);
                if (key.compareTo(rightDataBox) < 0) {
                    pageNum = node.getChildren().get(right);
                } else {
                    pageNum = node.getChildren().get(right + 1);
                }
                break;
            }
            int middle = (left + right) / 2;
            DataBox middleDataBox = node.getKeys().get(middle);
            int compare = key.compareTo(middleDataBox);
            if (compare < 0) {
                right = middle;
                continue;
            } else if (compare > 0) {
                ...
                pageNum = node.getChildren().get(middle + 1);
                break;
            }
        }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        BPlusNode node = getLowerNode(key, this);
        Optional<Pair<DataBox, Long>> putResOpt = node.put(key, rid);
        if (!putResOpt.isPresent()) {
            return putResOpt;
        }
        DataBox splitKey = putResOpt.get().getFirst();
        Long addNodePageNum = putResOpt.get().getSecond();
        return putLowerSplit(splitKey, addNodePageNum);
    }

    public Optional<Pair<DataBox, Long>> putLowerSplit(DataBox splitKey, Long addNodePageNum) {
        int addIndex = 0;
        while (addIndex < keys.size()) {
            if (splitKey.compareTo(keys.get(addIndex)) < 0) {
                break;
            }
            addIndex++;
        }
        if (addIndex == keys.size()) {
            keys.add(splitKey);
            children.add(addNodePageNum);
        } else {
            keys.add(addIndex, splitKey);
            children.add(addIndex + 1, addNodePageNum);
        }
        if (keys.size() > 2 * metadata.getOrder()) {
            //split
            int needToSplitIndex = metadata.getOrder();
            DataBox needToSplitKey = keys.get(needToSplitIndex);
            List<DataBox> newKeys = keys.subList(needToSplitIndex + 1, keys.size());
            List<Long> newChildren = children.subList(needToSplitIndex + 1, children.size());
            InnerNode splitNode = new InnerNode(metadata, bufferManager, newKeys, newChildren, treeContext);

            keys = keys.subList(0, needToSplitIndex);
            children = children.subList(0, needToSplitIndex + 1);
            sync();
            return Optional.of(new Pair<>(needToSplitKey, splitNode.getPage().getPageNum()));
        } else {
            sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            BPlusNode rightmostLowerNode = getChild(children.size() - 1);
            Optional<Pair<DataBox, Long>> splitNodeOption = rightmostLowerNode.bulkLoad(data, fillFactor);
            if (!splitNodeOption.isPresent()) {
                continue;
            }
            DataBox splitKey = splitNodeOption.get().getFirst();
            Long addNodePageNum = splitNodeOption.get().getSecond();
            Optional<Pair<DataBox, Long>> splitInnerNodeOption = putLowerSplit(splitKey, addNodePageNum);
            if (splitInnerNodeOption.isPresent()) {
                return splitInnerNodeOption;
            }
        }
        return Optional.empty();
    }
