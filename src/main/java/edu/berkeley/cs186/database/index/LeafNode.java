...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        //check need to split
        boolean needToSplit = keys.size() == 2 * metadata.getOrder();
        if (!needToSplit) {
            putWithOutSplit(key, rid, this);
            return Optional.empty();
        }
        int splitIndex = metadata.getOrder();
        LeafNode splitLeafNode = putWithSplit(key, rid, splitIndex);
        return Optional.of(new Pair<>(splitLeafNode.getKeys().get(0), splitLeafNode.getPage().getPageNum()));
    }

    private void putWithOutSplit(DataBox key, RecordId rid, LeafNode node) {
        //find insert index
        addRecordToKeysAndRids(key, rid, node.getKeys(), node.getRids());
        node.sync();
    }

    private LeafNode putWithSplit(DataBox key, RecordId rid, int splitIndex) {
        addRecordToKeysAndRids(key, rid, keys, rids);
        List<DataBox> newKeys = keys.subList(splitIndex, keys.size());
        List<RecordId> newRids = rids.subList(splitIndex, rids.size());
        LeafNode splitNode = new LeafNode(metadata, bufferManager, newKeys, newRids, rightSibling, treeContext);
    ...
    private void addRecordToKeysAndRids(DataBox key, RecordId rid, List<DataBox> keys, List<RecordId> rids) {
        int insertIndex = keys.size();
        for (int i = 0; i < keys.size(); i++) {
            DataBox ptr = keys.get(i);
            int compare = key.compareTo(ptr);
            if (compare == 0) {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int fillFactorSize = (int) (capacity * fillFactor);
        while (data.hasNext()) {
            Pair<DataBox, RecordId> next = data.next();
            if (keys.size() == capacity) {
                //need to split,and return
                LeafNode splitLeafNode = putWithSplit(next.getFirst(), next.getSecond(), fillFactorSize);
                return Optional.of(new Pair<>(splitLeafNode.getKeys().get(0), splitLeafNode.getPage().getPageNum()));
            }
            putWithOutSplit(next.getFirst(), next.getSecond(), this);
        }
        return Optional.empty();
    }
    ...
    public void remove(DataBox key) {
        // DONE(proj2): implement
        int index = Search.binarySearch(keys, key);
        if (index < 0) {
            return;
        }
        keys.remove(index);
    ...
    private void sync() {
        ...
        try {
            ...
            if (!Arrays.equals(bytes, newBytes)) {
                // TODO can replace toBytes() with newBytes?
