...
import edu.berkeley.cs186.database.utils.Search;
...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        int i = Search.binarySearch(leafNode.getKeys(), key);
        if (i < 0) {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        LeafNode leafNode = root.get(key);
        int nextIndex = 0;
        while (nextIndex < leafNode.getKeys().size()) {
            DataBox leafNodeKey = leafNode.getKeys().get(nextIndex);
            if (leafNodeKey.compareTo(key) >= 0) {
                break;
            }
            nextIndex++;
        }
        return new BPlusTreeIterator(leafNode, nextIndex);
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> putSplitOption = root.put(key, rid);
        if (!putSplitOption.isPresent()) {
            return;
        }
        handleAfterSplit(putSplitOption.get());
        return;
    }

    private void handleAfterSplit(Pair<DataBox, Long> splitKeyAndNewSplitPageNum) {
        DataBox splitKey = splitKeyAndNewSplitPageNum.getFirst();
        Long newSplitPageNum = splitKeyAndNewSplitPageNum.getSecond();
        List<DataBox> newKeys = new ArrayList<>();
        newKeys.add(splitKey);
        List<Long> newChildren = new ArrayList<>();
        newChildren.add(root.getPage().getPageNum());
        newChildren.add(newSplitPageNum);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (leftmostLeaf != root || leftmostLeaf.getKeys().size() > 0) {
            throw new BPlusTreeException("root page is not empty");
        }
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> splitKeyAndNewPageNumOption = root.bulkLoad(data, fillFactor);
            if (!splitKeyAndNewPageNumOption.isPresent()) {
                continue;
            }
            handleAfterSplit(splitKeyAndNewPageNumOption.get());
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private int nextRidIndex;

        public BPlusTreeIterator(BPlusTree tree) {
            leafNode = tree.root.getLeftmostLeaf();
            nextRidIndex = 0;
        }

        public BPlusTreeIterator(LeafNode leafNode, int nextRidIndex) {
            this.leafNode = leafNode;
            this.nextRidIndex = nextRidIndex;
        }
        ...
        public boolean hasNext() {
            // DONE(proj2): implement
            int leafNodeSize = leafNode.getKeys().size();
            if (nextRidIndex < leafNodeSize) {
                return true;
            }
        ...
        public RecordId next() {
            // DONE(proj2): implement
            if (hasNext()) {
                RecordId recordId = leafNode.getRids().get(nextRidIndex);
                nextRidIndex++;
                return recordId;
            }
