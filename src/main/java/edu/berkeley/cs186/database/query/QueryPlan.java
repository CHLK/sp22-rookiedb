...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int minOpIOCost = minOp.estimateIOCost();
        // TODO(proj3_part2): implement
        List<Integer> eligibleIndexColumns = getEligibleIndexColumns(table);
        Integer excludeIndex = -1;
        for (Integer eligibleIndexColumnIndex : eligibleIndexColumns) {
            SelectPredicate selectPredicate = this.selectPredicates.get(eligibleIndexColumnIndex);
            IndexScanOperator candidateOp = new IndexScanOperator(
                    this.transaction, selectPredicate.tableName,
                    selectPredicate.column,
                    selectPredicate.operator,
                    selectPredicate.value
            );
            int candidateIOCost = candidateOp.estimateIOCost();
            if (candidateIOCost < minOpIOCost) {
                minOpIOCost = candidateIOCost;
                minOp = candidateOp;
                excludeIndex = eligibleIndexColumnIndex;
            }
        }

        minOp = addEligibleSelections(minOp, excludeIndex);
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Map.Entry<Set<String>, QueryOperator> prev : prevMap.entrySet()) {
            Set<String> prevSet = prev.getKey();
            QueryOperator prevQueryOperator = prev.getValue();
            for (JoinPredicate joinPredicate : this.joinPredicates) {
                Set<String> passNumSet = new HashSet<>(prevSet);
                ...
                if (prevSet.contains(joinPredicate.leftTable) && !prevSet.contains(joinPredicate.rightTable)) {
                    rightOp = pass1Map.get(Collections.singleton(joinPredicate.rightTable));
                    rightColumn = joinPredicate.rightColumn;
                    newTable = joinPredicate.rightTable;
                    ...
                    rightOp = pass1Map.get(Collections.singleton(joinPredicate.leftTable));
                    rightColumn = joinPredicate.leftColumn;
                    newTable = joinPredicate.leftTable;
                    leftColumn = joinPredicate.rightColumn;
                } else {
                    continue;
                }
                passNumSet.add(newTable);
                QueryOperator passNumOp = minCostJoinType(prevQueryOperator, rightOp, leftColumn, rightColumn);
                if (result.containsKey(passNumSet)) {
                    QueryOperator existPassNumOp = result.get(passNumSet);
                    if (passNumOp.estimateIOCost() >= existPassNumOp.estimateIOCost()) {
                        continue;
                    }
                }
                result.put(passNumSet, passNumOp);
            }
        }
        return result;
    }
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> passNMap = pass1Map;
        for (int i = 1; i < this.tableNames.size(); i++) {
            passNMap = minCostJoins(passNMap, pass1Map);
        }

        for (QueryOperator op : passNMap.values()) {
            if (this.finalOperator == null) {
                this.finalOperator = op;
                continue;
            }
            if (this.finalOperator.estimateIOCost() > op.estimateIOCost()) {
                this.finalOperator = op;
            }
        }
        this.addGroupBy();
